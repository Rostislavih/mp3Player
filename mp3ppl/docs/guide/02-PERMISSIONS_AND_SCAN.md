...
1. Опишем модель `AudioTrack` в `:core`.
2. Добавим `Album` в `domain/model`.
3. Сделаем `expect class MusicScanner` в `core/commonMain`, Android-реализацию и iOS-заглушку.
...
7. Сделаем группировку `List<AudioTrack>` → `List<Album>`.

Файлы, которые появятся / изменятся:

```
core/src/
├── commonMain/kotlin/org/example/mp3player/core/audio/
│   ├── player/AudioTrack.kt              (новый, модель)
│   └── scanner/MusicScanner.kt           (новый, expect)
├── androidMain/kotlin/org/example/mp3player/core/audio/
│   └── scanner/MusicScanner.android.kt   (переделать + фикс бага)
└── iosMain/kotlin/org/example/mp3player/core/audio/
    └── scanner/MusicScanner.ios.kt       (новый, заглушка)

shared/domain/src/commonMain/kotlin/org/example/mp3player/domain/
├── model/Album.kt                        (новый)
├── repository/TracksRepository.kt        (новый)
└── repository/AlbumsRepository.kt        (новый)

shared/data/src/commonMain/kotlin/org/example/mp3player/data/repository/
├── TracksRepositoryImpl.kt               (новый)
└── AlbumsRepositoryImpl.kt               (новый)

composeApp/src/androidMain/
├── AndroidManifest.xml                   (добавить permission)
└── kotlin/.../permissions/AudioPermission.kt   (новый, Compose-обёртка)
```
...
### Шаг 1 — Модель `AudioTrack` в `:core`

Модель трека кладём в самый нижний модуль — она нужна и сканеру, и плееру, и всем слоям выше (почему именно так — разобрали в 01). Поле `albumId` обязательно: без него не сгруппировать треки в альбомы правильно.
...
Про `albumId`: два альбома с одинаковым названием, но разными исполнителями — это разные альбомы, и отличает их именно `albumId` (MediaStore присваивает его на уровне «название + исполнитель»).

Теперь прописываем зависимости в Gradle. `domain` отдаёт `:core` наружу (см. 01 про `api` vs `implementation`), а `data` подключает `domain`:

```kotlin
// shared/domain/build.gradle.kts
commonMain {
    dependencies {
        api(projects.core)                       // ← AudioTrack торчит в сигнатурах репозиториев
...
commonMain {
    dependencies {
        implementation(projects.shared.domain)   // :core приедет транзитивно
        implementation(libs.kotlinx.coroutines.core)
    }
}
```

Теперь импорт `org.example.mp3player.core.audio.player.AudioTrack` компилируется и в `data`, и позже в `presentation`.

### Шаг 2 — Добавить `Album` в `domain/model`
...
Создаём файл `shared/domain/.../model/Album.kt`:

```kotlin
// shared/domain/src/commonMain/kotlin/org/example/mp3player/domain/model/Album.kt
...
Главное про эту модель — она **не хранится** в БД и не собирается вручную. На Шаге 8 мы выведем `List<Album>` из `List<AudioTrack>` через `groupBy`. Сейчас просто фиксируем форму данных.

Заметки по полям:
- `id` — это `albumId` из MediaStore (строкой, чтобы единообразно с `AudioTrack.albumId`).
...
`Flow<List<AudioTrack>>` — это «горячая трубка»: каждый раз, когда список треков меняется, прилетает новый снимок. Подробнее `Flow` разберём в Шаге 7, где появится первая реализация.
...
```kotlin
// shared/domain/src/commonMain/kotlin/org/example/mp3player/domain/repository/AlbumsRepository.kt
package org.example.mp3player.domain.repository
...
actual class MusicScanner(private val context: Context) {
    ...
    actual suspend fun scanTracks(): List<AudioTrack> = withContext(Dispatchers.IO) {
        ...
        // дальше внутри withContext — projection (колонки запроса), selection (фильтр), query + обход курсора (сборка List<AudioTrack>), return tracks
    }
}
```
...
```kotlin
withContext(Dispatchers.IO) { scanTracks() }   // дожидается результата, возвращает List<AudioTrack>
...
```kotlin
    actual suspend fun scanTracks(): List<AudioTrack> = withContext(Dispatchers.IO) {
        ...
        // дальше — query к MediaStore и обход курсора для сборки List<AudioTrack>
    }
```
...
Сам запрос с обходом курсора и сборкой `AudioTrack`-ов:
...
```kotlin
// shared/data/src/commonMain/kotlin/org/example/mp3player/data/repository/TracksRepositoryImpl.kt
...
`asStateFlow()` — это **апкаст** до `StateFlow<List<AudioTrack>>`. Возвращается тот же самый объект `_tracks`, но через тип без setter'а. Снаружи никто не сможет вызвать `_tracks.value = ...` — только подписаться и читать.
...
- **`distinctUntilChanged` встроено.** Если новое значение `equals` старому — подписчики не получат повторный эмит. Поэтому `data class AudioTrack(...)` важен (у него правильный `equals`): если список после нового сканирования совпал с предыдущим — UI не будет зря перерисовываться.
...
```kotlin
// shared/data/src/commonMain/kotlin/org/example/mp3player/data/repository/AlbumsRepositoryImpl.kt
package org.example.mp3player.data.repository
...
**`observeTracks()`** возвращает `Flow<List<AudioTrack>>` — «провод», по которому каждый раз, когда `TracksRepositoryImpl` обновляет `_tracks.value`, прилетает свежий список.
...
class AlbumsRepositoryImpl(
    ...
    // дальше — private fun groupIntoAlbums(), для группировки List<AudioTrack> в List<Album> по albumId
}
```
...
Теперь — private-функция `groupIntoAlbums`, ради которой всё затевалось. Цепочка из трёх вызовов на `List<AudioTrack>`. Начнём с `groupBy`:
...
`groupBy` возвращает `Map<String, List<AudioTrack>>`. Ключ — `albumId` каждого трека, значение — список всех треков с этим `albumId`.

Конкретный пример. На устройстве 5 треков:

```
AudioTrack("1", title="Money",        albumId="42")
AudioTrack("2", title="Time",         albumId="42")
AudioTrack("3", title="Brain Damage", albumId="42")
AudioTrack("4", title="Wish You Were Here", albumId="7")
AudioTrack("5", title="Have a Cigar",       albumId="7")
```

После `groupBy { it.albumId }`:

```
"42" -> [AudioTrack("1", "Money", ...), AudioTrack("2", "Time", ...), AudioTrack("3", "Brain Damage", ...)]
"7"  -> [AudioTrack("4", "Wish You Were Here", ...), AudioTrack("5", "Have a Cigar", ...)]
```
...
Это **`Map.map { entry -> ... }`** из stdlib (НЕ `Flow.map`!). На входе — `Map.Entry<String, List<AudioTrack>>`, на выходе — `List<Album>`.
...
Как это будет использоваться (полный рабочий код — в главе 06, шаг 1.10; здесь — чтобы увидеть цепочку целиком; **писать пока некуда**):

```kotlin - пример (превью из главы 06 (1.10), сейчас не писать)
setContent {
    val (permissionState, requestPermission) = rememberAudioPermissionState()

    LaunchedEffect(permissionState) {
        if (permissionState == AudioPermissionState.Unknown) requestPermission()
    }

    when (permissionState) {
        AudioPermissionState.Granted -> RootScreen()          // весь UI приложения
        else -> { /* баннер «нужен доступ к музыке» + кнопка requestPermission */ }
    }
}
```

Вызывающий получает текущее состояние и функцию-триггер; `LaunchedEffect` спрашивает разрешение один раз при первом запуске; `when` показывает либо приложение, либо баннер.

`LaunchedEffect(permissionState)` — здесь ключ **не** `Unit`, и это правильно: мы хотим перезапустить эффект ровно тогда, когда статус разрешения изменился (пользователь нажал «Разрешить»). Ср. с главой 06, где для одноразового старта используется `LaunchedEffect(Unit)`.

> **Где физически живёт этот код.** `rememberAudioPermissionState()` использует `android.Manifest`, `ContextCompat`, `LocalContext` — это Android-only API, поэтому файл лежит в `composeApp/src/androidMain/`, а не в `commonMain` модуля `presentation`. Экраны из глав 06–07 кросс-платформенные и про разрешения не знают; связывает их **Android-точка входа** — `MainActivity`. Это и делается в главе 06, шаг 1.10.
>
> **Пока эта связка не написана — приложение никогда не спросит разрешение**, `MediaStore` вернёт пустой курсор, и список треков будет пустым независимо от того, сколько музыки лежит на устройстве. Не пропусти шаг 1.10.
...
1. **Проверь фикс бага**: временно добавь в конец `scanTracks()` строку `println("Found ${tracks.size} tracks")` и запусти приложение. В Logcat должно быть число больше нуля. (Экрана со списком ещё нет — он появится в главе 06.)

2. **Поиграйся с фильтром**: измени `DURATION > 10000` на `> 60000` — пропадут все треки короче минуты. Верни как было.

3. **Добавь поле `year`**: в `AudioTrack` добавь `val year: Int? = null`, в `projection` — `MediaStore.Audio.Media.YEAR`, прочитай и заполни.

4. **Группировка по исполнителю**: добавь метод `observeTracksByArtist(artist: String): Flow<List<AudioTrack>>` в `TracksRepository`. Реализация как `observeTracksOfAlbum`, но через `filter { it.artist == artist }`.

5. **Тест на эмуляторе**: треков там нет по умолчанию. Скачай пару MP3, положи через Device Explorer в `/sdcard/Music/`, перезапусти приложение. Если не видит — `MediaScannerConnection.scanFile(context, arrayOf(path), null, null)`.
